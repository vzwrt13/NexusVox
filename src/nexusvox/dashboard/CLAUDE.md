...
**Settings (tab 2)**
- Voice Input card: Hotkey Mode select (`hold` / `toggle`), `POST /api/settings/hotkey-mode`. Live: `HotkeyListener` reads the shared `Config` on every key event, no restart and no app bridge needed.
