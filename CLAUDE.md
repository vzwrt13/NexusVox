...
Modifier-only hotkey listener (pynput, Ctrl+Shift+Alt) triggers microphone capture (sounddevice, 16kHz PCM16 mono) which streams to a transcription backend (WebSocket for Voxtral/vLLM, HTTP POST for Cohere/Parakeet). Transcribed text passes through a nexus OS command check, then the assistant forward (`assistant.py`: "nexus assistant <text>" is sent to a local TCP port as `send <service> <text>` and not typed; unreachable port falls back to typing), then the correction dictionary (`dictionary.py`, entries in SQLite, edited in the dashboard), then optional voice command processing, then optional translation into English (`translator.py`: a local HTTP translator server; unreachable server falls back to the original text), then is injected at the cursor via clipboard + SendInput (Ctrl+V) with WM_PASTE fallback, and saved to SQLite as spoken, before translation. A Flask dashboard on port 47392 provides analytics, settings, flagging/review, and file upload transcription. System tray provides language toggle, translate toggle, dashboard access, and quit.
